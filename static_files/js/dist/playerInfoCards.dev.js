...
var twoElixir = ["Barbarian Barrel", "Bats", "Fire Spirits", "Giant Snowball", "Goblins", "Ice Golem", "Rage", "Spear Goblins", "The Log", "Wall Breakers", "Zap"];
var threeElixir = ["Archers", "Arrows", "Bandit", "Bomber", "Cannon", "Clone", "Dart Goblin", "Earthquake", "Elixir Golem", "Firecracker", "Fisherman", "Goblin Barrel", "Goblin Gang", "Guards", "Ice Wizard", "Knight", "Mega Minion", "Miner", "Minions", "Princess", "Royal Delivery", "Royal Ghost", "Skeleton Army", "Skeleton Barrel", "Tombstone", "Tornado"];
...
var eightElixir = ["Electro Giant", "Golem"];
var nineElixir = ["Three Musketeers"]; // Custom CSS styling for the cards table (small screen)

$("#cards_table").children("thead").css({
  "background-color": "#343a40",
  "color": "#ffffff"
});
$("#cards_table").find("th").css({
...
var directions = [1, Number.NEGATIVE_INFINITY, 1, 1, 1];
...
function sortTable(n) {
  ...
  var newTable;
  var rows = table.rows;
  var dir = directions[n]; // This code segment toggles the up and down arrows

  var upArrows = document.getElementsByClassName("up-arrow-sort");
  var downArrows = document.getElementsByClassName("down-arrow-sort");
  ...
  for (var _i = 1; _i < rows.length - 1; _i++) {
    var toAddIndex = _i;
    var toAdd = void 0;

    if (n === 0) {
      toAdd = rows[_i].getElementsByTagName("TD")[n].innerHTML.toLowerCase();
    } else {
      toAdd = rows[_i].getElementsByTagName("TD")[n].innerHTML;
    ...
    for (var j = _i + 1; j < rows.length; j++) {
      var toCheck = void 0;
    ...
    if (_i !== toAddIndex) {
      rows[_i].parentNode.insertBefore(rows[toAddIndex], rows[_i]);

      rows[_i + 1].parentNode.insertBefore(rows[_i + 1], rows[toAddIndex + 1]);
    }
  }
...
function sortCards() {
  ...
  switch (sortType) {
    ...
    case "Level":
      {
        $("#cardListDisplay").html($("#cardListDisplay").children().sort(function (a, b) {
          var num1 = parseInt($(a).find(".card-level").text().substring(6));
          var num2 = parseInt($(b).find(".card-level").text().substring(6));
    ...
    case "Upgrade":
      {
        $("#cardListDisplay").html($("#cardListDisplay").children().sort(function (a, b) {
          var text1 = $(a).find(".card-count").text();
          var text2 = $(b).find(".card-count").text();
          var percent1 = -1;
          var percent2 = -1; // Set large percent if card is already maxed
          ...
          if (percent2 === -1) {
            ...
            var _arr = text2.split(" ");

            percent2 = parseInt(_arr[0]) / parseInt(_arr[2]);
          }
    ...
    case "Rarity":
      {
        $("#cardListDisplay").html($("#cardListDisplay").children().sort(function (a, b) {
          var rarity1 = getRarity($(a).find("h5").text());
          var rarity2 = getRarity($(b).find("h5").text());
    ...
    case "Elixir":
      {
        $("#cardListDisplay").html($("#cardListDisplay").children().sort(function (a, b) {
          var elixir1 = getElixirCost($(a).find("h5").text());
          var elixir2 = getElixirCost($(b).find("h5").text());
    ...
    case "Trade":
      {
        $("#cardListDisplay").html($("#cardListDisplay").children().sort(function (a, b) {
          ...
          var text2 = $(b).find(".card-count").text();
          var trade1 = -1;
          var trade2 = -1; // Set large trade if card is already maxed
          ...
          if (trade2 === -1) {
            ...
            var _arr2 = text2.split(" ");

            trade2 = getTrades(parseInt(_arr2[0]), getRarity($(b).find("h5").text()), parseInt($(b).find(".card-level").text().substring(6)));
          }
    ...
    case "Star Level":
      {
        $("#cardListDisplay").html($("#cardListDisplay").children().sort(function (a, b) {
          var starLevel1 = $(a).find("img").length - 1;
          var starLevel2 = $(b).find("img").length - 1;
          var level1 = parseInt($(a).find(".card-level").text().substring(6));
          var level2 = parseInt($(b).find(".card-level").text().substring(6)); // Ensures that non-maxed cards come before maxed cards in the sort
