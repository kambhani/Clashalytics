function loadLeaderboard(caller) {
  const gtTag = $(caller).children("p").text();
  $(".spinner-border").removeClass("d-none");
  fetch(`/tournaments/gt/${gtTag}/leaderboard`)
    .then(res => res.text())
    .then((text) => {
      if (text === "Error") {
        ...
        $("#leaderboardDiv").html(text);
        // Have to call bootstrap toggle initializers since the html is dynamically loaded
...
$(window).resize(function() {
  ...
  // Takes care of screen resizing for the filter feature on the leaderboard
  if ($("#gtLeaderboard").length) {
    if (window.matchMedia("(min-width: 768px)").matches) {
      if ($("#gtLeaderboardFilter1").val() === "" || $("#gtLeaderboardFilter1").parent().find("button").text().includes("Filter By")) {
        ...
        $("#filterDisplay").text(`Showing ${$("#gtLeaderboard").children().length} of ${$("#gtLeaderboard").children().length} placements`);
      } else {
        filterLeaderboard(1);
      }
    } else {
...
function sizeLeaderboardIcons() {
  const smallSize = "25px";
  const largeSize = "40px";
  if (window.matchMedia("(min-width: 576px)").matches) {
    $(".gt-leaderboard-icon-div").css("height", largeSize);
    $(".gt-leaderboard-icon-div img").css("height", largeSize);
    $(".gt-leaderboard-icon-div i").css({"font-size": largeSize, "line-height": largeSize});
    ...
    $(".gt-leaderboard-icon-div i").css({"font-size": smallSize, "line-height": smallSize});
...
$("#leaderboardDiv").on("click", ".dropdown-item-sort", function(e) {
  ...
  if (parentButton.text() !== selectedText) {
    ...
    let dropdowns = $(this).parent().parent().parent().children(".dropdown");
    ...
    if (!dropdowns.get(0).innerText.includes("Sort Type") && !dropdowns.get(1).innerText.includes("Sort Order")) {
      ...
      switch (sortType) {
        ...
        case ("Rank"): {
          ...
          toSort.html(toSort.children().sort(function (a, b) {
            let num1 = parseInt($(a).find(".gt-leaderboard-rank").text());
            let num2 = parseInt($(b).find(".gt-leaderboard-rank").text());
            if (num1 > num2) {
        ...
        case ("Losses"): {
          toSort.html(toSort.children().sort(function (a, b) {
            let num1 = parseInt($(a).find(".gt-leaderboard-losses").text());
            let num2 = parseInt($(b).find(".gt-leaderboard-losses").text());
            if (num1 < num2) {
              return (sortOrder === "Ascending") ? -1 : 1;
            } else if (num1 > num2) {
              return (sortOrder === "Ascending") ? 1 : -1;
        ...
        case ("Rank Change"): {
          toSort.html(toSort.children().sort(function (a, b) {
            let num1 = parseInt($(a).find(".gt-leaderboard-rank-change").text());
            let num2 = parseInt($(b).find(".gt-leaderboard-rank-change").text());
            if (num1 < num2) {
              return (sortOrder === "Ascending") ? -1 : 1;
            } else if (num1 > num2) {
              return (sortOrder === "Ascending") ? 1 : -1;
...
function filterLeaderboard(callerId) {
  let id = "#gtLeaderboardFilter" + callerId;
  let sortBy = $(id).parent().find("button").text();
  let filter = document.getElementById(id.substring(1)).value.toUpperCase();
  ...
  let totalParticipants = $("#gtLeaderboard").children().length;
  let displayedParticipants = totalParticipants - $("#gtLeaderboard").children(".d-none").length;
  $("#filterDisplay").text(`Showing ${displayedParticipants} of ${totalParticipants} participants`);
}