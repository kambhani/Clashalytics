...
$(".dropdown-item-sort").on("click", function(e) {
  ...
  if (parentButton.text() !== selectedText) {
    ...
    if (!dropdowns.get(0).innerText.includes("Sort Type") && !dropdowns.get(1).innerText.includes("Sort Order")) {
      let toSort = $("#tournamentParticipants");
      switch (sortType) {
        case ("Name"): {
        ...
        case ("Rank"): {
          toSort.html(toSort.children().sort(function (a, b) {
            let num1 = parseInt($(a).find(".tournament-rank").text());
            let num2 = parseInt($(b).find(".tournament-rank").text());
            if (num1 < num2) {
...
function filterTournamentStandings(callerId) {
  let id = "#tournamentFilter" + callerId;
  ...
  $("#tournamentParticipants div").each(function(index) {
    switch (sortBy) {
      case "All": {
        let str = $(this).find(".tournament-player-name").text().toUpperCase() + " " + $(this).find(".tournament-clan-name").text().toUpperCase() + " " + $(this).find(".tournament-rank").text().toUpperCase() + " " + $(this).find(".tournament-wins").text().toUpperCase();
  ...
  let totalParticipants = $("#tournamentParticipants").children().length;
  let displayedParticipants = totalParticipants - $("#tournamentParticipants").children(".d-none").length;
...
$(window).resize(function() {
  if (window.matchMedia("(min-width: 768px)").matches) {
    if ($("#tournamentFilter1").val() === "" || $("#tournamentFilter1").parent().find("button").text().includes("Filter By")) {
      ...
      $("#filterDisplay").text(`Showing ${$("#tournamentParticipants").children().length} of ${$("#tournamentParticipants").children().length} participants`);
    } else {
