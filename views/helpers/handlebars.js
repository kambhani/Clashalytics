...
var register = function(Handlebars) {
  ...
  var helpers = {
    ...
    getClanBadge(id, cwTrophies, clanBadgeJson) {
    ...
    averageElixirCost(cards, deck, start, end) {
      ...
      let toReturn = Math.round(sum / (end - start) * 10) / 10;
    ...
    getGtTierPicture(tierObject) {
      switch (tierObject.type) {
        case ("tradeToken"): {
          return `/images/resources/webp/trade_token_${tierObject.rarity}.webp`;
        }
        case ("resource"): {
        ...
        case ("chest"): {
          switch (tierObject.chest) {
            ...
            case "Giant":
            case "Lightning": {
              return `/images/chests/webp/${tierObject.chest} Chest.webp`;
            }
            // Special Exceptions
            case ("Kings"): {
    ...
    getGtTierPictureAltText(tierObject) {
      switch (tierObject.type) {
        case ("tradeToken"): {
          return `${tierObject.rarity} trade token`;
        }
        case ("resource"): {
          if (tierObject.resource === "gold") {
            return "Gold";
    ...
    // 0 is for lower case
    // 1 is for upper case
    convertTextToCase(text, textCase) {
      if (typeof text === "undefined") {
        return "Server Error";
      }
      switch (textCase) {
        case 0: return text.toLowerCase();
